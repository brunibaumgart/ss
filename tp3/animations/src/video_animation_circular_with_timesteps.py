...
def create_animation_video(sim_params, data, output_video, delta_t):
    ...
    enclosure_radius = int((L / 2) * scale)  # El radio del recinto en píxeles
    center = (resolution // 2, resolution // 2)  # El centro del recinto es el centro del frame
    ...
    for i, (current_time, particles) in enumerate(data):
        if current_time - last_time >= delta_t:
            ...
            cv2.circle(frame, center, enclosure_radius, (0, 0, 0), 3)
