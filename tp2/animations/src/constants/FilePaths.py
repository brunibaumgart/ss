SIMULATIONS_DIR = "../../simulations/src/main/resources/"
VIDEO_FRAMES_DIR = SIMULATIONS_DIR + "video_frames/"

PARAMETERS_FILE = SIMULATIONS_DIR + "parameters.txt"
VIDEO_PARAMETERS_FILE = SIMULATIONS_DIR + "video_parameters.txt"

TIME_VS_VA_FILE = SIMULATIONS_DIR + "time_vs_va.txt"
VA_VS_ETHA_FILE = SIMULATIONS_DIR + "va_vs_etha.txt"

OUTPUT_DIR = "../output/"
OUTPUT_DEFAULT_VIDEO = OUTPUT_DIR + "default_animation.mp4"

OUTPUT_TIME_VS_VA = OUTPUT_DIR + "time_vs_va.png"
